  ...
  return (
    <>
      <div className="font-display absolute top-0 left-0 z-20 w-full h-full flex flex-col items-center justify-center imagen bg-blue-500">
        <p
          className="text-sm text-white z-20 w-3/4 md:w-1/2 xl:w-1/4 flex gap-2 items-center pb-3 cursor-pointer"
          onClick={() => router.back()}
        >
          {" "}
          <ArrowIcon className="w-3 h-3 transform rotate-90" /> Volver a atrás
        </p>
        ...
        <p className="text-white text-xs z-20 pt-10">
...
const Providers = () => {
  const Provider = ({ provider }) => {
    const providers = {
      ...
      google: {
        color: "red",
        icon: <GoogleIcon className="text-white w-6 h-6" />,
      },
      instagram: {
        color: "gray",
    ...
    return (
      <div
        className={`border rounded-full w-12 h-12 grid place-items-center bg-${providers[provider]?.color}-500 hover:opacity-95 transition hover:scale-105 transform cursor-pointer`}
      >
  ...
  return (
    <div>
      <h2 className="text-sm text-gray-400 w-full text-center pb-2">
        Accede con
      </h2>
